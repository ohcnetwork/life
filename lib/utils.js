...
export const isVerified = (verificationStatus) => {
    return verificationStatus && verificationStatus.toLocaleLowerCase() == 'verified';
};
...
const extractPhoneNumbersFromString = (phoneString) => {
    if (phoneString && phoneString !== null && phoneString != '') {
        ...
        return phones.map(number => number.split(',').length > 1 ? number.toString().replace(/[,]+/g, '') : number);
    }
    return [];
};
