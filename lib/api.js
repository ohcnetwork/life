...
import { activeStates, parametreize, statePaths } from "./utils";
...
export const districtWithState = () => statePaths("all");

export function getDistricts(state) {
  
  return statePaths("all").filter((f) => parametreize(f.state) == state);
}
