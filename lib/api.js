...
import hospitalObjects from '@data/hospital_clinic_centre.json';
import helplineObjects from '@data/helpline.json';
import oxygenRequirements from '@data/requirement_data.json';
import vaccineObjects from '@data/vaccine.json';
...
export function getVaccine(state, district, isSortingRequired) {
    return new get(vaccineObjects).from(state, district, isSortingRequired);
}
